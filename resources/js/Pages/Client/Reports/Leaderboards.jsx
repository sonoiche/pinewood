...
const Leaderboards = ({ events }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    await axios.post('/client/reports/generate-leaderboards', data).then((response) => {
  ...
  return (
    <AuthenticatedLayout>
      <Head title="Race Category Summary Leaderboard" />
        ...
        <div className="bg-white rounded-xl shadow-lg">
          <div className="px-6 py-4 border-b border-gray-200 bg-gradient-to-r from-indigo-200 to-indigo-50">
            <div className="flex items-center justify-between">
              <div>
                <h1 className="text-2xl font-bold text-gray-800">Race Category Leaderboard</h1>
                <p className="text-gray-600 mt-1">Shows top racers in a race category, ranked by total points (or average time as a tiebreaker).</p>
              </div>
            </div>
          </div>
              ...
              {category.name ? (
                <>
                  <div className="mx-5 my-5">
                    <h2 className="text-blue-600 text-xl uppercase tracking-wider">Leaderboard - {category.name}</h2>
                  </div>
                    ...
                    <table className="w-full border border-gray-300 rounded-md">
                      ...
                      <tbody>
                        {Array.isArray(summaries) && summaries.length > 0 ? (
                          ...
                          <tr>
                            <td className='border border-gray-300 px-3 py-1.5 text-gray-600 text-base tracking-wide text-center'>{index+1}</td>
