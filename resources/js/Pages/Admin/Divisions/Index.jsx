...
const DivisionIndex = ({ divisions }) => {

  const handleEdit = (id) => {
    router.get(`/client/divisions/${id}/edit`);
  }

  const handleDelete = (id) => {
    if(confirm('Are you sure you want to delete this?')) {
      router.delete(`/client/divisions/${id}`);
    }
  }
    ...
    <AuthenticatedLayout title="">
        ...
        <div className="bg-white rounded-xl shadow-lg overflow-hidden">
          <div className="px-6 py-4 border-b border-gray-200 bg-gradient-to-r from-indigo-200 to-indigo-50">
            <div className="flex items-center justify-between">
              <div>
                <h1 className="text-2xl font-bold text-gray-800">List of Divisions</h1>
                <p className="text-gray-600 mt-1">Manage Divisions</p>
              </div>
              <div className="flex space-x-2">
                <Link href="/client/divisions/create" className="px-4 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 transition-colors duration-200 flex items-center">
            ...
            <table className="w-full">
              <thead>
                <tr className="text-left text-gray-700 bg-gray-100">
                  ...
                  <th className="px-6 py-3 font-semibold uppercase tracking-wider w-2/6">Division</th>
              ...
              <tbody className="divide-y divide-gray-200">
                {divisions.map((division, index) => (
                  <tr className="hover-animate">
                    ...
                    <td className="px-6 py-4 text-gray-600">{division.name}</td>
                    <td className="px-6 py-4 text-right">
                      <div className="flex justify-end space-x-2">
                        <button className="p-2 text-blue-500 hover:text-blue-700 transition-colors cursor-pointer" onClick={() => handleEdit(division.id)}>
                          <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
                            <path d="M13.586 3.586a2 2 0 112.828 2.828l-.793.793-2.828-2.828.793-.793zM11.379 5.793L3 14.172V17h2.828l8.38-8.379-2.83-2.828z" />
                          </svg>
                        </button>
                        <button className="p-2 text-red-500 hover:text-red-700 transition-colors cursor-pointer" onClick={() => handleDelete(division.id)}>
                          <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
                            <path
                              fillRule="evenodd"
      ...
      <div id="emptyState">
        <div className="py-12 text-center">
          ...
          <p className="mt-1 text-sm text-gray-500">Get started by adding a new division.</p>
          <div className="mt-6">
            <Link
              href="/client/divisions/create"
